...
def read_orig_50salads_annotations(videos: list, root: Path,
    ...
    print("videos: ", videos)
    print("root: ", root)
    print("annots_dir: ", annots_dir)
    print("timestamps_dir: ", timestamps_dir)
    all_segments = []
    for video in videos:
        video_fname, annot_fname = bundle_entry_to_video_fname_50salads(
            video.strip(), root)
        print("video_fname, annot_fname", video_fname, annot_fname)
        ...
        with open(Path(annots_dir) / annot_fname, 'r') as fin:
            ...
            for line in fin:
                ...
                print("(video_fname, start, end, label):", video_fname, start, end, label)
    return all_segments
...
def get_past_segment_inputs(df):
    """
    custom: past segment inputs dump
    """
    seq_labels = []
    for i, df_row in df.iterrows():
        df_video = df[df.video_path == df_row.video_path]
        labels = [row['action_class'] for idx, row in df_video.iterrows() if idx < i]
        if len(labels) > 0:
            dedup_labels = []
            for label in labels:
                if len(dedup_labels) == 0 or label != dedup_labels[-1]:
                    dedup_labels.append(label)
            seq_labels.append(labels)

    with open('label_input_seq.pickle', 'wb') as handle:
        pickle.dump(seq_labels, handle)
    with open('label_input_seq.pickle', 'rb') as handle:
...
def init_df(bundle_fpath: Path, annot_reader_fn: TargetConf, root: Path,
    ...
    # get_past_segment_inputs(dataframe) # custominfo
    return dataframe

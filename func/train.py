...
def train_one_epoch(
    ...
    for i, data in enumerate(
        ...
        optimizer.zero_grad()
        # loss.backward() #commenting to avoid load on CPU
...
def main(cfg):
    ...
    print("custom here after num_classes = {key: len(val) for key, val in dataset.classes.items()}")
    ...
    if cfg.train.init_from_model:
        ...
        ckpt_path = cfg.train.init_from_model
    ...
    if cfg.test_only:
        ...
        evaluate(train_eval_op, data_loaders_test, writer, logger)
        return
    ...
    for epoch in range(start_epoch, cfg.train.num_epochs):
        ...
        last_saved_time = train_one_epoch(train_eval_op,
                                          optimizer,
                                          lr_scheduler,
                                          data_loader,
                                          epoch,
                                          ...
                                          1,
                                          {'max_norm': None},
                                          {'past_cls_action': 1.0, 'feat': 1.0, 'cls_action': 1.0, 'distill': 1.0},
                                          1,
                                          45,
                                          True)
        # print_freq,
